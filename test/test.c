...
void validate(void){
  ...
  begin_section("BF");
  //+++[-]
  assert_eq((run("1+++??[-??]"),  M[0]), 0);
  //+++
  assert_eq((run("1+++"), M[0]), 3);
  //+++++[>>+<<-] copy value to 2 cells to the right
  assert_eq((run("1+++++??[>>+<<-??]"), M[2]), 5);
  //+++++>+++[<+>-] addition 5 + 3
  assert_eq((run("1+++++>+++??[<+>-??]"), M[0]), 8);
  //+++++++>+++++[<-] subtraction 7 - 2
  assert_eq((run("1+++++++>+++++??[<->-??]"), M[0]), 2);
  //+++[>+++++<-] multiplication 3 * 5
  assert_eq((run("1+++??[>+++++<-??]"), M[1]), 15);
  //+[>>+>+<<<-]>>>[<<<+>>>-]>+<<[-----[>]>>[<<<+++>>>[-]] if(x == 5){y = 3}
  assert_eq((run("1+++++??[>>+>+<<<-??]>>>??[<<<+>>>-??]>+<<??[-----??[>??]>>??[<<<+++>>>??[-??]??]"), M[1]), 3);
  assert_eq((run("1++++??[>>+>+<<<-??]>>>??[<<<+>>>-??]>+<<??[-----??[>??]>>??[<<<+++>>>??[-??]??]"), M[1]), 0);
  assert_eq((run("1++++++??[>>+>+<<<-??]>>>??[<<<+>>>-??]>+<<??[-----??[>??]>>??[<<<+++>>>??[-??]??]"), M[1]), 0);
  end_section();
}
